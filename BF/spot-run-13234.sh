...
#SBATCH --job-name=spot-run-13234
#SBATCH --output=/sdf/home/a/abrought/run5/BF/output/spotout-run-13234.txt
#SBATCH --error=/sdf/home/a/abrought/run5/BF/output/spoterr-run-13234.txt
#SBATCH --ntasks=1
...
pipetask run \
        -j 25 \
        -d "instrument='LSSTCam' AND detector in ( 23 ) AND exposure.observation_type='spot' AND exposure.science_program IN ('13234')" \
        -b ${REPO}/butler.yaml \
        -i LSSTCam/raw/all,LSSTCam/calib,${sbias},${sdark},${sflat},${defects},${ptcs},${bfks} \
        -o u/abrought/BF/run_13234/R03_S12/corrected \
        -p ${YAML}/gridFit.yaml \
        --register-dataset-types
 
cp ${YAML}/gridFit.yaml ${REPO}/u/abrought/BF/run_13234/R03_S12/corrected
...
pipetask run \
        ...
        -b ${REPO}/butler.yaml \
        -i LSSTCam/raw/all,LSSTCam/calib,${sbias},${sdark},${sflat},${defects},${ptcs},${bfks} \
        -o u/abrought/BF/run_13234/R03_S12/uncorrected \
        -p ${YAML}/gridFit2.yaml \
        --register-dataset-types
 
cp ${YAML}/gridFit2.yaml ${REPO}/u/abrought/BF/run_13234/R03_S12/uncorrected
